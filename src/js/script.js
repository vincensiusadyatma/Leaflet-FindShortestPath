...
class Graph {
    ...
    createVertex({ id = null, vertexType, lat, lon, label = null }, insert = false) {
        if (!Object.values(VertexTypes).includes(vertexType)) {
            throw new Error(`Invalid vertexType: ${vertexType}`);
        }

        if (vertexType === VertexTypes.INTERSECTION) {
            const newVertex = new Vertex(this.getThenIncrementedVertexId(), 'intersection', lat, lon, this, label);
        } else {
            const newVertex = new Vertex(id, vertexType, lat, lon, this, label)
        }
...
class Vertex {
    ...
    constructor(id, vertexType, lat, lon, parent, label = null) {
        if (!Object.values(VertexTypes).includes(vertexType)) {
            throw new Error(`Invalid vertexType: ${vertexType}`);
        }
